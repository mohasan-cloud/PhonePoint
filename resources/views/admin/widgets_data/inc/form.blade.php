    ...
    @for($i = 1 ; $i <= $wid->extra_fields; $i++)
        ...
        <div class="col-md-12">
            <div class="form-group">
                {!! Html::label($name, $placeholder, ['class' => 'font-weight-bold']) !!}
                {!! Html::text($name, null, ['class' => 'form-control', 'id' => $name, 'placeholder' => $placeholder, 'required' => 'required']) !!}
            </div>
        </div>
    @endfor
@endif
  ...
  @if($wid->is_description)
    <div class="col-md-12">
        <div class="form-group">
            {!! Html::label('description', $wid->title.' Description', ['class' => 'font-weight-bold']) !!}
            {!! Html::textarea('description', null, ['class' => 'form-control', 'id' => 'description', 'placeholder' => $wid->title.' Description', 'required' => 'required']) !!}
        </div>
    </div>
@endif
