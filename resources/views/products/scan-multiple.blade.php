                ...
                <div class="col-lg-6 col-md-6 col-12">
                    <div class="card mb-4 shadow-sm">
                        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('Shop.bill') }}" class="btn btn-light btn-sm">
                                <i class="fas fa-plus"></i> Shop Bill
                            </a>
                        </div>
                        ...
                        <div class="card-body">
                            <form id="payment-form">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select id="discount" class="form-control" onchange="updatePriceAfterDiscount()">
                                        <option value="">No Discount</option>
                                        <!-- Add dynamic discounts here -->
                                ...
                                <div class="mb-3">
                                    <label for="price-after-discount" class="form-label">Price After Discount</label>
                                    <input type="text" id="price-after-discount" class="form-control" readonly>
                                </div>
                                <div class="mb-3">
...
function updateProductList(scannedBarcode, product) {
    ...
    if (productMap[scannedBarcode]) {
        ...
        newRow.innerHTML = `
            <td>${product.name}</td>
            <td>
                <input type="number" class="form-control product-price-input" value="${product.discounted_price.toFixed(2)}" 
                    min="${product.buy_price.toFixed(2)}" step="0.01" 
                    data-buy-price="${product.buy_price.toFixed(2)}">
            </td>
        ...
        const priceInput = newRow.querySelector('.product-price-input');
        priceInput.addEventListener('change', () => validatePrice(priceInput));
...
function validatePrice(priceInput) {
    const buyPrice = parseFloat(priceInput.dataset.buyPrice);
    const inputPrice = parseFloat(priceInput.value);
    const scannedBarcode = Object.keys(productMap).find(key => productMap[key].row.contains(priceInput));
    ...
    if (inputPrice < buyPrice) {
        Swal.fire({
            icon: 'error',
            title: 'Invalid Price',
            text: `The price cannot be less than the buy price: ${buyPrice.toFixed(2)}.`,
        });
        priceInput.value = oldPrice.toFixed(2); // Reset to the old price
    } else {
        // Update total price
        const priceDifference = inputPrice - oldPrice;
        totalPrice += priceDifference * quantity;
        totalPriceElement.textContent = totalPrice.toFixed(2);
